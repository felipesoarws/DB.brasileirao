...
export function Team({id,name,color,reverse=false,shortNameOverride}:{id:string,name:string,color?:string|null,logo?:string|null,reverse?:boolean,shortNameOverride?:string}){
  ...
  const shortName=shortNameOverride||(words.length>1?words.map(word=>word[0]).join('').toUpperCase():words[0]?.slice(0,3).toUpperCase()||'—');
...
export function StandingsTable({rows,compactColumns=false}:{rows:Row[];compactColumns?:boolean}){
