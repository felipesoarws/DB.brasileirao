    ...
    <div className="match-stat-columns" role="tabpanel">{statColumns.map((column,index)=><div className="match-stat-column" key={`stats-column-${index}`}>
      {column.map(categoryKey=>{
        ...
        return <section className="match-stat-category" key={category.key}><h3>{category.label}</h3>{rows.map(stat=>{
          const home=Number(stat.homeValue)||0,away=Number(stat.awayValue)||0,total=home+away,homeShare=total?home/total*100:0,awayShare=total?away/total*100:0;
...
function ScorePredictions({forecast,homeId,awayId,homeName,awayName,homeScore,awayScore,finished}:{forecast:MatchForecast;homeId:string;awayId:string;homeName:string;awayName:string;homeScore:number|null;awayScore:number|null;finished:boolean}){
  ...
  return <section className="match-detail-section card match-predictions" aria-label="Probabilidades e expectativas de gols">
    <div className="match-prediction-layout">
      <div className="match-prediction-probabilities"><div className="match-section-heading"><div><span className="match-section-kicker">PROBABILIDADES</span><h2>Placares mais prováveis</h2></div><div className="match-score-help-wrap"><button type="button" className="match-forecast-help" aria-label="Como são calculadas as probabilidades dos placares?" aria-expanded={showScoreInfo} aria-controls="match-score-explanation" onClick={()=>setShowScoreInfo(value=>!value)}>?</button>{showScoreInfo&&<div className="match-forecast-popover" id="match-score-explanation" role="dialog" aria-label="Como são calculadas as probabilidades dos placares"><strong>Como calculamos</strong><p>Calculamos uma expectativa de gols para cada time a partir dos resultados anteriores e do desempenho no mando, dando mais peso aos jogos recentes.</p><p>Com essas expectativas, usamos uma distribuição de Poisson para estimar a chance de cada placar. A lista mostra os cinco placares com maior probabilidade; é uma estimativa estatística, não uma garantia do resultado.</p></div>}</div></div>
