  ...
  return <>
    ...
    {tab==='estatisticas'&&<>
      <section className="section season-statistics"><div className="statistics-section-heading"><span className="statistics-section-icon">◉</span><div><h2>Panorama de gols</h2><p className="caption">Resumo da temporada até as partidas finalizadas disponíveis.</p></div></div>{analytics?.stats?<><Kpis items={[{label:'Partidas finalizadas',value:analytics.stats.matches},{label:'Gols marcados',value:analytics.stats.totalGoals},{label:'Média de gols por jogo',value:analytics.stats.goalsPerMatch.toFixed(1)},{label:'Gols de mandantes',value:analytics.stats.homeGoals},{label:'Gols de visitantes',value:analytics.stats.awayGoals}]}/><div className="season-stat-grid">{[
        ['Mais de 1,5 gol',analytics.stats.over15],['Mais de 2,5 gols',analytics.stats.over25],['Mais de 3,5 gols',analytics.stats.over35],['Ambos marcaram',analytics.stats.bothScored],['Ao menos um time sem sofrer gol',analytics.stats.cleanSheets]
      ].map(([label,value])=>{const percent=analytics.stats.matches?Math.round(Number(value)/analytics.stats.matches*100):0;return <div className="card season-stat-card" key={String(label)}><div><strong>{label}</strong><span className="numeric">{value} <small>({percent}%)</small></span></div><div className="season-stat-track"><span style={{width:`${percent}%`}}/></div></div>})}</div></>:<div className="empty">Estatísticas ainda não disponíveis para esta temporada.</div>}</section>
